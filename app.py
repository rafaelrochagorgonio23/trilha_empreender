...
# Requer: gspread, google-auth no requirements.txt
...
SHEET_ID = st.secrets.get("sheets", {}).get("sheet_id", "1tJUnK8kqe9uyRTCq20uSN5gswv1_0PpR1cMpxLq72gk")  # fallback
ws = client.open_by_key(SHEET_ID).worksheet("A")  # TODO: troque "respostas" pelo nome real da aba (se for "A", use "A")
...
if st.button("Gerar trilha (detalhes)"):
    # Unifique a assinatura da função para (perfil, objetivo, tempo, area)
...
# ==== AÇÃO 2: Gerar recomendação + salvar no Sheets ====
